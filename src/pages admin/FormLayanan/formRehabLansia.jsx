...
function FormAdminRehabLansia() {
    ...
    const handleSubmit = async () => {
        // Validation checks
        if (!formData.NIK || !formData.kebutuhan || formData.ktp.length === 0 || formData.kk.length === 0 ) {
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
        ...
        try {
            ...
            const response = await api.post('/admin-upload-rehabilitasi-lansia', formDataToSend, {
                headers: {
            ...
            navigate('/admin/layanan/rehabilitasi-lansia');
        } catch (error) {
...
export default FormAdminRehabLansia;
