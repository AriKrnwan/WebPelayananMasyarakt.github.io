...
function DetailRehabAnak() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await api.get(`/lay-rehabilitasi-anak-terlantar/${nopel}`, {
                    headers: {
    ...
    const handleUpdate = async () => {
        ...
        if (suketFile) formData.append('suket_kelurahan', suketFile);
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await api.put(`update-rehabilitasi-anak-terlantar/${nopel}`, formData, {
                headers: {
                    Authorization: `Bearer ${token}`,
    ...
    const handleValid = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await api.put(`update-rehabilitasi-anak-terlantar/${nopel}`, {
            ...
            const notifResponse = await api.post('/notifikasi', {
                ...
                no_lay: data.no_lay,
                layanan: "rehabilitasi-anak-terlantar",
                type_message: 1
            }, {
                headers: {
                    'Authorization': `Bearer ${token}`
    ...
    return (
        <>
            ...
            <InputFile
                ...
                filePath={data && data.id ? data.id : ''}
                table={layanan}
                onChange={(e) => setSuketFile(e.target.files[0])}
                disabled={status !== "Menunggu Validasi"}
            />
...
export default DetailRehabAnak;
