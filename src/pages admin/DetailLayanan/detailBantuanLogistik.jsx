...
function DetailBantuanLogistik() {
    ...
    const handleUpdate = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await api.put(`update-bantuan-logistik/${nopel}`, formData, {
                headers: {
                    Authorization: `Bearer ${token}`,
    ...
    const handleValid = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await api.put(`update-bantuan-logistik/${nopel}`, {
    ...
    const handleInvalid = async (typeMessage) => {
        ...
        try {
            ...
            Swal.fire({
                title: "Good job!",
                text: typeMessage === 2 ? "Status Pengajuan Menjadi Tidak Valid" : "Pengajuan Ditolak",
                icon: "success"
            });
        } catch (error) {
...
export default DetailBantuanLogistik;
