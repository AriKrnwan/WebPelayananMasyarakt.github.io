...
function DetailPUB() {
    ...
    const handleUpdate = async () => {
        ...
        if (materaiFile) formData.append('surat_pernyataan_bermaterai', materaiFile);
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await api.put(`update-pengumpulan-uang-dan-barang/${nopel}`, formData, {
                headers: {
                    Authorization: `Bearer ${token}`,
    ...
    const handleValid = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await api.put(`update-pengumpulan-uang-dan-barang/${nopel}`, {
            ...
            const notifResponse = await api.post('/notifikasi', {
                ...
                no_lay: data.no_lay,
                layanan: "pengumpulan-uang-dan-barang",
                type_message: 1
            }, {
                headers: {
                    'Authorization': `Bearer ${token}`
    ...
    return (
        <>
            ...
            />
            <InputFile
                label="Rurat Keterangan bagi Ormas"
                name="suket_ormas"
                showDownloadButton
                filePath={data && data.id ? data.id : ''}
                table={layanan}
            ...
            />
            <InputFile
                label="Surat Keterangan bagi LKS"
                name="suket_lks"
                showDownloadButton
                filePath={data && data.id ? data.id : ''}
                table={layanan}
            ...
            />
            <InputFile
                label="Surat Keabsahan Legalistas"
                name="surat_legal"
                showDownloadButton
                filePath={data && data.id ? data.id : ''}
                table={layanan}
            ...
            <InputFile
                ...
                filePath={data && data.id ? data.id : ''}
                table={layanan}
                onChange={(e) => setMateraiFile(e.target.files[0])}
                disabled={status !== "Menunggu Validasi"}
            />
