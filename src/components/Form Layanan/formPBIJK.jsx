...
function FormPBIJK({ disabled }) {
    ...
    const handleUpdate = async () => {
        ...
        if (suketFile) formData.append('suket_rawat_inap', suketFile);

        try {
            const token = localStorage.getItem('token');
            const response = await api.put(`/update-PBI-JK/${data.no_lay}`, formData, {
                headers: {
                    Authorization: `Bearer ${token}`,
    ...
    const handleDelete = async () => {
        try {
            const token = localStorage.getItem('token');
            await api.delete(`/delete-PBI-JK/${data.no_lay}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
